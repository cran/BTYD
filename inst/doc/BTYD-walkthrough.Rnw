...
\section{Version 2.4.2 Overview}
...
The patched BTYD functions have been modified as follows: 
\begin{itemize}
...
\item Some of the functions stored in \texttt{R/bgnbd.R} have been changed in order to implement the fix for the \texttt{NUM!} error problem proposed in \href{http://www.brucehardie.com/notes/027/bgnbd_num_error.pdf}{this note}. 
\item As of version 2.4.2 the \texttt{hardie} argument defaults to TRUE with the exception of \texttt{bgnbd.generalParams} where it defaults to NULL.
\end{itemize}
